...
var Agrro_Timer = 0  #Timer for how long the enemy will chase the player till it gives up and stops.
var Chase = false
...
func _on_alerting_area_body_exited(_body: Node2D) -> void: #Player is not in view
	$AggroTimer.start()
	Chase = true
	Agrro_Timer = 0 #Restarting the Aggro_Timer if not in range
	print(Agrro_Timer)

func _on_timer_timeout() -> void: #three second timer
	if Agrro_Timer <= 9 and Chase == true: #Plusing it once each time the timer goes off
		Agrro_Timer += 1
		$AggroTimer.start()
		print(Agrro_Timer)
	if Agrro_Timer == 10: #If the Agrro_Timer reaches 10 than we reset the timer and unalert the enemy
		Alert = false
